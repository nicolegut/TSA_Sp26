...
acf_tren  <- Acf(ts_data[,1])
pacf_tren <- Pacf(ts_data[,1])
...
acf_hydro  <- Acf(ts_data[,2])
pacf_hydro <- Pacf(ts_data[,2])
...
```{r}
"The series Total Renewable Energy Production appears to have an upwards linear trend with potential seasonality. The series Hydroelectric Power Comsumption appears to have a strong seasonal pattern."
```
...
detrend_inflow_data <- inflow_data[,iHP+1] - (beta1 + beta2*t)
...
#monthly data (freq 12), starts at year 1931, month 1

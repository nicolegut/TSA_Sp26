...
#can set the test used to decide if there is a unit root/ if differencing is needed
#kpss is the default --> would say "KPSS test showed that there is a unit root so we differenced the series"
...
model_011011 <- Arima(ts_electricity_price[,"Residential"],
                     order = c(0,1,1), 
                     seasonal = c(0,1,1))
print(model_011011)
checkresiduals(model_011011)
compare_aic <- data.frame(model_011011$aic)

model_111011 <- Arima(ts_electricity_price[,"Residential"],
                     order = c(1,1,1), 
                     seasonal = c(0,1,1))
print(model_111011)
checkresiduals(model_111011)
compare_aic <- data.frame(compare_aic, model_111011$aic)

model_010011 <- Arima(ts_electricity_price[,"Residential"],
                     order = c(0,1,0), 
                     seasonal = c(0,1,1))
print(model_010011)
checkresiduals(model_010011)
compare_aic <- data.frame(compare_aic, model_010011$aic)

model_012011 <- Arima(ts_electricity_price[,"Residential"],
                     order = c(0,1,2), 
                     seasonal = c(0,1,1))
print(model_012011)
checkresiduals(model_012011)
compare_aic <- data.frame(compare_aic, model_012011$aic)  
...
##model with the deseasoned data is technically the lowest but on different data 
##cannot compare models on different data
...
model_autofit_nonseas <- auto.arima(deseasonal_residential_price,
                                    max.D = 0,
                                    max.P = 0,
                                    max.Q = 0, 
                                    approximation = TRUE)
print(model_autofit_nonseas)
compare_aic <- cbind(compare_aic, model_autofit_nonseas$aic)

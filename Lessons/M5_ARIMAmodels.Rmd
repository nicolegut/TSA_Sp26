...
#ACF decays relatively slowly/ very high first lag --> indicates that there is a trend that might need removal
...
# large p value --> reject null hypthosesis --> ts has a unit root --> need to difference
...
autoplot(deseasonal_residential_price_diff)
...
# no more decay in the ACF --> do not need to do any more differencing
...
#high PACF at lag 1 is also gone --> need to specify d=1 when fitting arima model to time series (just provide arima with the deseasoned series, say d=1, rather than deseason and difference and provide that to the model, because the arima can do the differencing)
```
...
## 1,1,1 -> no cut off/ tail off --> mixed model/ starting with the simplest model, the middle 1 refers to differencing
## 
...
print(Model_111)
#drift is the new mean of the series after differencing it
#one AR, one MA term
compare_aic <- data.frame(Model_111$aic)
#storing AIC's for all models that we are testing --> for later comparison
checkresiduals(Model_111)
#shows plot of residuals/ ACF/ histogram of residuals --> normally distributed around 0
...
Model_112 <- Arima(deseasonal_residential_price,
                   order = c(1,1,2),
                   include.drift = TRUE 
                   )
print(Model_112)
...
Model_212 <- Arima(deseasonal_residential_price,
                   order = c(2,1,2),
                   include.drift = TRUE 
                   )
print(Model_212)
...
## the 212 model has a lower AIC value --> but coefficients are very close to 1/ -1
## coefs close to abs(1) can indicate covariance
```

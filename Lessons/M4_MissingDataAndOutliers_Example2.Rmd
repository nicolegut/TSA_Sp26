...
## two methods for looking at outliers in the data --> can plot histograms/ box and whisker
## then can see the distributions are from a log normal distribution
## will never have observations that are negative/ only really high
...
Series with seasonality/ strong trends --> need to remove first before outlier removal
- because seasonal outliers need to be compared with seasonal means not the overall mean 
    - overall mean would be too conservative of a metric for outliers
- strong increasing/ decreasing trend --> mean of the series wold have the same issues. 
...
```{r}
# decompose function uses the moving average for the trend
# seasonal component is the same is the seasonal means model
# can just deseason a series through subtracting the seasonal component from the series
# CANNOT do that with the trend though --> still need to fit a linear model

# when series does not have a seasonal component --> seasonal component will look incredibly messy/ not work
```
...
#has date column and random inflow part
...
```
Data is ready! Will have 6 NA's in the front and the end of the trend series, because order 12 moving average needs at least 6 points in front and behind the average. Because the trend series has lost values --> the random component will also lose values.

Need to exclude values using na.omit

> Answer: Why are values so low for the random series --> calculating mean/ stdev of the random component. Random has seasonal and trend already removed from the original series, so the series should be close to zero. 
...
## adding the +- 3 std around the mean lines in the plot --> to see if there are outliers
## if values fall outside of the 3std, might need to remove outliers
## could use the ts_clean to remove outliers.
```
...
#on a seasonal dataset --> should replace with the local mean
# ts_clean function will do this for all outliers --> probably better to do manually/ don't remove too many

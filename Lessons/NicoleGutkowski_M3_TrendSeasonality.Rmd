...
```{r}
#Create vector t
t <- c(1:nobs) #dataset 960

#choose 1 hp to work with
iHP <- 3


#Fit a linear trend to TS of iHP
# iHP + 1 because the first column is date
...
beta1 <- as.numeric(linear_trend_model$coefficients[1]) #intercept
beta2 <- as.numeric(linear_trend_model$coefficients[2]) #slope
...
From the regression on HP15 --> non significant trend (0.3557), so no overall linear trend
reg on HP4 --> signif trend p-value: < 2.2e-16, overall linear trend
...
#only two of the months are significant --> seasonality not as strong for this 
#dataset, or maybe has different seasonal patterns?
...
#season_inflow_data <- beta_int+beta_coeff*dummies
...
ts_season <- ts(season_inflow_data, 
                start = c(1931,1),
                frequency = 12)

#plots
autoplot(ts_inflow_data[,iHP], series = "Original Inflow Data")+
  autolayer(ts_deseason_inflow_data, series = "Detrend and Deseason Inflow")+
  autolayer(ts_season, series = "season time series")

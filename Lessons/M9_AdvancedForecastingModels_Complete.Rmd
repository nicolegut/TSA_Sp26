...
pro_act_power_daily <-
  act_power_daily %>% 
  rename(ds=Date,y=daily_mean_active_power) %>% 
  select(ds,y)

nobs <- nrow(pro_act_power_daily)

pro_act_power_daily_train <-
  pro_act_power_daily[1:(nobs-n_for),]

# Fit model
Proph_fit <- prophet(pro_act_power_daily_train)

# Create future dataframe (365 days ahead)
future <- make_future_dataframe(Proph_fit, periods = 365)

# Fit model
Proph_for <- predict(Proph_fit,future)

#Plot foresting results
plot(Proph_fit, Proph_for)


ggplot() +
  
  # Actual data as LINE (not points)
  geom_line(data = pro_act_power_daily, 
            aes(x = ds, y = y), 
            linewidth = 0.8,
            color="black") +
  
  # Forecast line
  geom_line(data = Proph_for, 
            aes(x = ds, y = yhat), 
            linewidth = 1,
            color="blue") +
...
Proph_for_ts <- msts(Proph_for$yhat,
                   start=c(2006,12,16),
                   seasonal.periods=c(7,365.25))
                   
Proph_for_ts <- tail(Proph_for_ts,365)

#Plot model + observed data
autoplot(ts_act_power_daily) +
  autolayer(Proph_for_ts, series="Prophet",PI=FALSE)+
  ylab("Active Power") 
```


## Checking accuracy of the forecasting models
...
# Model 5:  Prophet 
Prophet_scores <- accuracy(Proph_for_ts,ts_act_power_daily_test)
...
scores <- as.data.frame(
  rbind(ETS_scores, ARIMA_scores, TBATS_scores, NN_scores, Prophet_scores)
  )
row.names(scores) <- c("STL+ETS", "ARIMA+Fourier","TBATS","NN","Prophet")
...
Prophet is the best model by RMSE, but ARIMA + Fourier is the best by MAPE.
...
autoplot(ts_act_power_daily) +
  ...
  autolayer(Proph_for_ts,PI=FALSE, series="Prophet") +

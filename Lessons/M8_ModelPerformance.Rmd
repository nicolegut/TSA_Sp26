...
> meanf is the arithmetic mean
> setting range of the mean-ing
> only using the overall mean as forecasting
> doesn't get rid of the trend well/ residuals are not normally distributed
> in the check residuals --> Ljung-box test --> reject the null hypothesis that the residuals are independent --> shows that the trend was not removed
...
> Residuals are a little bit better than they were
> Still a strong autocorrelation in the ACF --> still a trend in the data
> Ljung box test --> resid still not independent
...
> autofits an arima model to the dataset
> residuals are normally distributed and the ljung box test we cannot reject null
...
**object** object of class forecast, or numerical values containing forecasts.
> for the naive and snaive --> object will be class forecast
> for the autoarima --> need to convert into the forecast class object

**x** numerical vector containing observed values (optional).
...
#creating vector with the observed values (the final 12 that we left out to forecast)
observed <- electricity_price_processed[(nobs-n_for+1):nobs, "All.sectors"]

# ---------------------------------------------------------------------
#Model 1: Arithmetic mean
#MEAN_seas is the object that we created with the arithmetic mean function
#need to select the correct thing to plot --> the mean
MEAN_scores <- accuracy(MEAN_seas$mean,observed)  #store the performance metrics
#providing the accuracy function with the 12 forecast and the actual observed values
...
#creating the forecast object using the model itself NOT the ts object
...
ts_electricity_price_full <- ts(
  electricity_price_processed[1:nobs,"All.sectors"],

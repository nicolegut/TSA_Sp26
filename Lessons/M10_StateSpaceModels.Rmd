...
Help the model converge to a better approach? --> former model had seasonality in the errors and the 
```{r}
SS_seas <- StructTS(ts_price_train ,
...
All underestimating the true values --> what to do
- with the BSM --> will most likely update level term
--> try the BSM model --> forecast the last 6 observations instead of last 12 and see what models are better
- BSM need to calibrate the varince

Where to start when estimating variance
- ES/ SEAS --> will est based on first few values/ don't need to worry about it
- can use the first year ish of data to get prior beliefs?
- even if prior beliefs are off then they will eventually be fixed

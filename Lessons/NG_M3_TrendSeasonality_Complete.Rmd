...
None of the series have stochastic trends --> generally linear trends that we could inform with a linear model.
...
Pacf(ts_inflow_data[,iHP],lag.max=40,main=paste("Inflows HP",iHP,sep=""))
...
```
Comparing original and deseasoned data

## Let's change the order of the process, i.e, fit seasonal first and then trend
## what happens when you do seasonal and then trend!
## should always do trend and then seasons
#if there is a trend --> will impact seasons
...
##running mann kendal on yearly dataset and printing the results
print(summary(MannKendall(inflow_data_yearly)))

```
The spearman is less than one/ p value is less than 0.05 --> there is a deterministic trend in the dataset / rejecting null hypothesis. When looking at seasonal data --> might not be an overall trend but could be seasonal trends (colds getting colder and hots getting hotter etc)
- might cancel out/ not see otherwise

SMK test --> season 1/ season2/ season3 S/ var/ tau/ p values --> to see trends between years for the same month
...
print(sp_rho)
```
if close to 1 --> significant increasing trend
0.28 --> still signif diff than 0 but not a strong slope
...
first one: p val 0.01 --> reject null hypothesis/ series has a unit root
- tells me that there is no stochastic trend
- saw that there was a deterministic trend from the other tests

Second one: same thing! p val 0.045 ish
